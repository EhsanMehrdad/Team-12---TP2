...
?>
 <?php
 if( isset($_POST['subb'])&& !empty($_POST['search'])){
    $search=$_POST['search'];
        $row= $db->prepare("SELECT * From products WHERE category LIKE '%$search%' ");
        $row->execute();
        $serchp=$row->fetchAll(PDO::FETCH_ASSOC);
 }else{
...
<nav class="navbar navbar-expand-lg" id="navbar">
  ...
  <?php foreach ($serchp as $sp):?>
    <div class="col-md-3 py-3 py-md-0" style="margin-bottom: 30px;" >
      <a href="kwajo.php?page=productdet&pid=<?=$sp['pid']?> &model=<?=$sp['model']?>" ><div class="card">
        <img src="./images/<?=$sp['img']?>" alt="">
        <div class="card-body">
          <h3 class="text-center"><?=$sp['name']?></h3>
          ...
          <h2><?=$sp['price']?> <span><li class="fa-solid fa-cart-shopping"></li></span></h2>
        </div>
      </div></a>
    </div>
    <?php endforeach;?>
